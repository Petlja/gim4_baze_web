...
Веб-апликације обично имају више страна и која страна ће бити
приказана зависи од путање која се наведе у прегледачу. На пример, ако
се веб-апликација налази на URL адреси
``http://www.moja-prodavnica.com``, посетом тој адреси се отвара
долазна страна апликације (енгл. landing page), а ако се приступи
адреси ``http://www.moja-prodavnica.com/proizvodi``, приступа се
страни са приказом свих производа. Када се у прегледачу приступи некој
таквој адреси шаље се захтев ка серверу. Сервер коме се захтев шаље се
одређује на основу имена домена, коришћењем сервиса DNS, а путања се
уписује у HTTP захтев, да би сервер тј. веб-апликација која је на њему
покренута знао коју страну треба да пошаље као одговор. На пример,
када се приступи URL адреси
``http://www.moja-prodavnica.com/proizvodi``, део захтева ће бити и
путања ``/proizvodi``.
...
за веб-страном. Која ће од тих функција бити позвана, зависи од самог
захтева тј. зависи од URL-а који је коришћен за приступ нашој
веб-апликацији. Flask врши тзв. **рутирање** тј. анализира захтев и
путању уписану у тај захтев и на основу тога позива неку од
дефинисаних Python функција. Да би функција била повезана са одређеним
обликом захтева (тј. URL-a) потребно је пре њене дефиниције навести
анотацију ``@app.route(...)``. У приказаном примеру, анотацијом
``@app.route("/")``, функција ``index`` је повезана са захтевом у ком
је наведена путања ``"/"``, што значи да ће та функција бити позвана
када се наведе адреса нашег веб-сајта, без икавких додатних путања
унутар URL-a (кажемо да је ово адреса корена нашег веб-сајта). То
значи да ће функција ``index`` бити позвана ако се Flask апликација
покрене на локалном рачунару помоћу ``flask run`` (без постављања на
неки сервер на интернету), и ако се у прегледачу откуца URL адреса
``http://127.0.0.1:5000/``. Исто би се десило и ако би се наша
апликација поставила на интернет, на пример на домен
...
Пошто функција ``index`` враћа ниску ``Zdravo`` тј. је одговор који се
шаље назад прегледачу и који се приказује.

Испред сваке функције је могуће навести и више различитих ``route``
анотација, чиме се та функција повезује са свим наведеним путањама.

Као што смо рекли, веб-апликација обично одговара на захтеве више
различитих путања. У наредном примеру се дефинишу две функције, при
...
.. mchoice:: 01_flask_rutiranje
   ...
   Којом анотацијом треба анотирати функцију која ће се позвати када
