...
на веб-страници и након попуњавања формулара на веб-страници, ти
подаци се преносе веб-апликацији која их онда обрађује (на пример,
складишти у базу података).
...
приликом свих захтева за страницама у којима се не преносе никакви
подаци (на пример, када отворите страницу ``http://petlja.org``, ваш
прегледач шаље GET захтев серверу ``petlja.org`` тражећи његову
почетну страницу, при чему не шаље никакве додатне податке).
...
такве веб-странице се могу појавити међу резултатима претраге
претраживача веба и слично.

Опишимо сада како Flask скрипт може да приступи подацима послатим у
склопу GET захтева. За то се користи објекат ``request`` библиотеке
Flask (који је, такође, потребно навести у склопу директиве
``import``). Методом ``request.args.get(...)`` скрипт може прочитати
вредност неког податка, на основу задатог назива тог
податка. Размотримо наредни пример, који садржи основну функционалност
која би се употребила у склопу претраге производа у некој електронској
веб-продавници.
...
Очекује се да се методом GET скрипту проследе два податка:

- ``vrsta`` означава врсту производа који се претражују,
- ``max_cena`` означава њихову највишу допуштену цену.

Пошто је могуће да се скрипт позове и без навођења тих података, у
скрипту се обезбеђују и њихове подразумеване вредности. Наиме, метода
``request.args.get`` најчешће има два аргумента.

- Први представља назив податка који се очитава из GET захтева (назив
  кључа).
- Други представља вредност која ће се вратити ако случајно податак
  тог назива није прослеђен у склопу GET захтева. Ако се тај аргумент
  изостави, а податак није прослеђен, променљива ће имати специјалну
  вредност ``None``.

Тако ће у случају коришћења URL-а
``http://127.0.0.1/pretraga&vrsta=TV`` вредност променљиве ``vrsta``
бити ``TV``, док ће у случају URL-а ``http://127.0.0.1/pretraga``
вредност те променљиве бити ``svi_proizvodi``. Ако променљива
``max_cena`` није прослеђена у склопу GET захтева (као аргумент),
променљива ``max_cena`` у скрипту ће имати вредност бесконачно (у
језику Python она је наведена помоћу израза ``float("inf")`` и то је
број који ће бити већи од било ког другог броја).
...
од 1 до 10 GET аргументом који одређује димензију те таблице. Нека се
кључ тог аргумента назива ``n``. Ако се тај аргумент не проследи,
...
наглашава да се вредност GET аргумента ``n`` конвертује у целобројну
...
захтев се шаље ка страници која је тренутно отворена).
...
Дефинише се путања ``/calculator``, што значи да страници на локалном
рачунару приступамо путем URL-а
``http://127.0.0.1/calculator``. Унутар функције ``calculator`` која
се тада покреће покушавамо да прочитамо вредности GET аргумената
...
``render_template`` да на основу шаблона формира страницу и тако
формирану страницу враћамо клијенту.
...
На почетку странице се налази формулар (елемент ``form``) за унос
...
захтев са URL-ом
``http://127.0.0.1/calculator?sab1=3&sab2=5``.
...
се унутар аргумената захтева налази кључ ``naruci`` (то је име
дугмента ``input type="submit"``, па ће бити део GET аргумената кад
год се то дугме притисне). Очитавамо један по један податак помоћу
``request.args.get``. Додатке (кечап, оригано итд.)  смештамо у листу.
