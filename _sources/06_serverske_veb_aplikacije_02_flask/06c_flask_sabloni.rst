...
Веб-апликације као одговор најчешће шаљу HTML стране. Навоћење HTML
кода у облику ниски у програмском језику је веома компликовано,
непрегледно и тешко за одржавање. Стога већина језика за писање
веб-апликација омогућава да се HTML кôд сними у посебне датотеке
(тзв. **шаблоне**) и касније прочита и обради помоћу скрипта написаног
у том програмском језику.

Приметимо да одговор који је наша северска апликација послала клијенту
не представља чак ни исправан HTML запис. Стога функцију ``index``
можемо проширити тако да врати целокупну HTML страну.

.. literalinclude:: /../_src/02_hello_html/app.py
    :language: python

Приметимо да смо ниску која садржи изворни HTML кôд навели између
наводника облика ``"""..."""`` уместо између наводника облика
``"..."`` или ``'...'``, чиме је омогућено да се ниска простире кроз
више редова.

Мешање HTML и Python кода у истој датотеци је веома лоша пракса. Стога
библиотека ``Flask`` користи библиотеку ``Jinja``
(https://jinja.palletsprojects.com/), која омогућава да се HTML кôд
чува у посебним датотекама (тзв. **шаблонима**, енгл. **templates**) и
да Python програм коришћењем тих шаблона једноставно креира HTML
стране које ће се слати као одговор клијенту. Библиотека ``Flask``
подразумева да се сви шаблони чувају у директоријуму ``templates``.
Претходни програм се може прилагодити на следећи начин. Нашу
апликацију ће чинити две датотеке: датотека ``app.py`` у кореном
директоријуму апликације и шаблон ``index.html`` који се налази у
његовом поддиректоријуму ``templates``.
...
Са друге стране датотека ``templates/index.html`` има следећи садржај:
...
директиве ``import``). Она је та која чита садржај наведене датотеке
(у овом случају то је ``index.html``), обрађује њен садржај и враћа
резултујућу ниску.
...
Приметимо да елемент ``h1`` не садржи неки конкретан наслов, већ му је
садржај ``{{ naslov }}``. То значи да приликом обраде шаблона,
функција ``render_template`` треба да тај садржај замени вредношћу
именованог аргумента ``naslov``, који јој је прослеђен приликом позива
(у овом примеру то ће бити ниска ``Predmeti``). Генерално, када се
између заграда ``{{`` и ``}}`` пронађе неки Python израз, он се мења
својом вредношћу. Нагласимо да ови изрази ипак не могу да садрже
позиве произвољних Python операција и функција, већ само неких
основних. Ипак, постоји начин да се омогући коришћење и функција које
нису аутоматски подржане, али о њему неће бити речи у овом уџбенику.
...
Примећујемо велико преклапање између два шаблона. Ствар је погоршана
тиме што се користи библиотека Bootstrap, тако да се и у једном и у
...
.. mchoice:: 02_flask_sabloni_4
   ...
   које се прликом обраде шаблона мењају својим вредностима?
