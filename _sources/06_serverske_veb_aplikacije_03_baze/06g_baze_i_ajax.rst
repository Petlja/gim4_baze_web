...
страницу и шаље је клијенту који ту страницу приказује у прегледачу
веба, мењајући њоме страницу која је била отворена. У овом поглављу
ћемо приказати другачију организацију апликације у којој клијент,
помоћу скрипта написаног у програмском језику JavaScript, на основу
неке акције корисника над тренутно отвореном страницом шаље захтев
серверу да му достави неке податке, а онда када ти подаци стигну
приказује их у склопу странице која је тренутно отворена.  Оqвај начин
рада се назива **AJAX** (*Asynchronous JavaScript and XML*). Разлог је
то што су некада подаци са сервера ка клијенту стизали у формату
*XML*, док се данас јако често користи формат *JSON* (*JavaScript
Object Notation*).
...
страница које садржи поље за претрагу. У датотеци ``app.py`` се налази
следећа функција.
...
на путањи ``/artists`` и који као GET аргумент прима текст уписан у
...
Овај серверски скрипт, дакле, прима GET аргумент ``name`` (то је ниска
карактера којом мора да почне назив извођача). Након тога формира упит
коришћењем израза ``Name LIKE 'name%'``, где је ``name`` почетак
назива извођача, док специјални знак ``%``, подсетимо се, у језику SQL
означава произвољну ниску карактера. За ово, је наравно, потребно
користити параметризовани упит. Иако би се могло помислити да је
параметризовани упит могуће направити тако што се на место почетка
назива извођача стави упитник. тј. тако да се у упиту напише ``Name
...
оператора ``LIKE`` буде параметар, тј. да се у упиту напише ``Name
...
параметризованих упита, аргумент морамо навести у облику једночлане
...
аргумент проследити ниску ``"GET"``, а као други параметар URL на који
ће се послати захтев. Да не бисмо зависили од путање, поново ћемо
унутар шаблона употребити функцију ``url_for`` којом ћемо добити
путању на којој одговара Python функција ``artists``. Иако параметре
URL-а можемо да изградимо и ручно (изразом ``"name=" +
inputName.value``), препоручени начин је да користимо уграђену
JavaScript функционалност за то. Класа ``URLSearchParams`` ће
изградити део URL-а који кодира GET аргументе, ако јој те аргументе
проследимо као речник. Након изграње URL-а и отварање везе према њему
методом ``open``, захтев коначно шаљемо методом ``send()``.
