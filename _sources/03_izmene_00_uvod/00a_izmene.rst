...
``SELECT`` и да на основу њих доносите одлуке битне за ваш посао.  

Поред упита ``SELECT``, језик ``SQL`` омогућава и упите за упис нових података
у табеле (упити ``INSERT``), за измену постојећих података (упити
``UPDATE``) и за брисање врста табеле (упити ``DELETE``).

Сви ови упити мењају садржај базе података, односно појединих табела у бази.
У овом поглављу ћемо видети како се користе ови упити.